...
#for spk in $spk_list; do
	...
	for sp in $sp_list; do
		...
		for utt in $utt_list; do
			...
			copy-feats ark:$dir/$spk/$utt.ark ark,t:- | uniq > $dir/sp$sp'-'$spk/$utt.ark
			sed "s/ ]//g" $dir/sp$sp'-'$spk/$utt.ark > $dir/sp$sp'-'$spk/$utt.ark.tmp
			rm $dir/sp$sp'-'$spk/$utt.ark
			mv $dir/sp$sp'-'$spk/$utt.ark.tmp $dir/sp$sp'-'$spk/$utt.ark
			...
			vframe_time=`echo "scale=5;40/$sp" | bc`
			st=`echo "scale=2;20/$sp" | bc`
			video_idx=1
			tmp=$dir/sp$sp'-'$spk/$utt.ark
				# duplicate utt id
			echo sp$sp'-'`cat $tmp | head -1 | tail -1` >> $tmp.tmp
			for idx in $(seq 1 $frame_sp); do
				current_time=`echo "scale=2;$st+$frame_time*$idx" | bc`

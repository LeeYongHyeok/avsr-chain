...
sed -e s/sox\ -t\ wav//g $data/wav.scp | awk '{print $1,$2}' > $logdir/video.scp
sed -i 's/.\/wav/.\/video/g' $logdir/video.scp
sed -i 's/.wav$/.ark/g' $logdir/video.scp
sed -i '16501,33000s/\/id/\/sp0.9-id/' $logdir/video.scp
sed -i '33001,49500s/\/id/\/sp1.1-id/' $logdir/video.scp

# compute the byte offset of the data in the ark file based on the length of the utterance id
cat $logdir/video.scp | cut -f1 | awk '{ print (length($1)+1) }' > $logdir/video_offs
